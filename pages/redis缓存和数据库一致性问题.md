...
- 梳理总结
  ...
  |缓存方案|评价|缓存读思路|缓存和DB写思路|缓存利用率|原子性问题|读写并发问题|DB主从复制延迟问题|优化手段|
  |---|---|---|---|---|---|---|---|---|
  |先更新缓存再更新DB|待评价|
  |先更新DB再更新缓存|待评价|
  |先删除cache再删除DB|待评价|
  |**先删除DB再删除cache**|相对最好的方案|
  |全量数据刷到缓存|最特别的一种方案|只读缓存,缓存不存在则返回|只写DB,定时任务异步更新缓存|cache永不失效，很低|不存在|数据不一致问题很严重|存在数据不一致问题|
  
  上述方案都不能解决强一致性问题，仍然存在一段时间内的数据不一致性问题,
- 过程
