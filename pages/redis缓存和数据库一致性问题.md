...
- 几种缓存方案大梳理总结
  ...
  |优化手段||||针对原子性问题,采用重试策略,引入消息队列/cancal中间件binlog方式;针对读DB从库+DB主从复制延迟下的读写并发问题,采用延时双删策略,二次删除,引入延时消息队列,延时二次删除缓存,具体延时时长大小视情况(主从延时状况)而定,虽然无法完全避免LastQ,但可以大大减少异常情况下LastQ的时长|暂无|
  |数据不一致性问题|||||数据不一致问题很严重|
