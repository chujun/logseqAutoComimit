...
- 1. 记录锁（Record Lock） ：也被称为记录锁，属于单个行记录上的锁。
  记录锁是锁在索引记录上的锁。(来自官网的说明)
  记录锁触发条件:查询条件命中索引，并且索引是唯一索引，同时查询到符合条件的记录,例如主键id,如果索引不是唯一索引触发的就是临键锁了。
- 2. 间隙锁（Gap Lock） ：又叫区间锁，锁定一个范围，不包括记录本身。
  当我们查询数据用范围查询而不是相等条件查询时，**查询条件是命中索引字段，并且没有查询到符合条件的记录**，此时就会将查询条件中的范围数据进行锁定(即使是范围库中不存在的数据也会被锁定)
  间隙锁触发条件:查询条件命中索引字段，并且没有查询到符合条件的记录
  间隙锁特征
  1. 间隙锁锁定区间范围是左开右开区间(后面有专门的实验证明，而非左开右闭区间)
  2. 间隙锁只会出现在可重复读的事务隔离级别中
- 3. 临键锁（Next-key Lock） ：Record Lock+Gap Lock，锁定一个范围，包含记录本身。记录锁只能锁住已经存在的记录，为了避免插入新记录，需要依赖间隙锁。
  临键锁触发条件:查询条件命中索引字段,同时有匹配到记录
  **临键锁是由记录锁和间隙锁共同实现的**
  特征:
...
- InnoDB锁实验
  ...
  ```
  第三个实验:间隙锁
  网络上常有一种说法临键锁锁定的区间范围是左开右闭区间。
  间隙锁的区间范围是左开右开区间。
  
  第一个session窗口执行如下命令,条件是索引字段，但是没有满足条件的记录
  ```
  begin;
  update innodb_lock_test set money=200 where id>1 and id<3;
  ```
  ...
  | ENGINE | ENGINE_LOCK_ID                          | ENGINE_TRANSACTION_ID | THREAD_ID | EVENT_ID | OBJECT_SCHEMA | OBJECT_NAME      | PARTITION_NAME | SUBPARTITION_NAME | INDEX_NAME | OBJECT_INSTANCE_BEGIN | LOCK_TYPE | LOCK_MODE | LOCK_STATUS | LOCK_DATA |
  +--------+-----------------------------------------+-----------------------+-----------+----------+---------------+------------------+----------------+-------------------+------------+-----------------------+-----------+-----------+-------------+-----------+
  | INNODB | 140233293675848:1613:140233557539392    | 66356                 | 79        | 16       | lock_test     | innodb_lock_test | <null>         | <null>            | <null>     | 140233557539392       | TABLE     | IX        | GRANTED     | <null>    |
  | INNODB | 140233293675848:356:4:3:140233568248352 | 66356                 | 79        | 16       | lock_test     | innodb_lock_test | <null>         | <null>            | PRIMARY    | 140233568248352       | RECORD    | X,GAP     | GRANTED     | 3         |
  +--------+-----------------------------------------+-----------------------+-----------+----------+---------------+------------------+----------------+-------------------+------------+-----------------------+-----------+-----------+-------------+-----------+
  ```
  锁信息如下
  1. 表锁意向排他锁
  ...
  4. 间隙锁，锁定基于(3,’2345@qq.com', 3)的间隙范围(注意不包含前后区间点，区间点由记录锁锁住了)
  ![image.png](../assets/image_1655265058494_0.png)
  
  根据上图分析可知:另一个session窗口此时唯一可插入的数据是(8,'2345@qq.com'),可以更新id=3的记录(因为并没有id=3的记录锁)
  关于判断是否是区间点需要主要的是，索引字段和主键字段合起来作为区间，(2,'2345@qq.com')在GAP间隙内，而(8,'2345@qq.com')在GAP间隙外
  实际另一个窗口执行效果如下
  ```
  begin;
  ...
  实验6:证明谬论:网上经常说的临键锁的锁定区间范围是左开右闭区间是不对的。
  本质还是临键锁=记录锁和
-
-
- 资料
