...
- 类加载器
  ...
  类加载器在多个领域大放异彩，最早期的Applet，JSP,现在的OSGI,程序热部署,代码加密等领域。
	- 类加载器类型:
	  ...
	  关于BootstrapClassLoader可以加载的类还有安全限制:类库名必须是jvm指定的,否则即使放在lib目录也不会被加载（按照文件名识别，如rt.jar、tools.jar)
	  应用程序类加载器是ClassLoader类中的getSystem-ClassLoader()方法的返回值,所有也被称为”系统类加载器"(注意不要以为是启动类加载器)
	- 类与类加载器的关系
	  jvm类加载器+类唯一决定一个类.
