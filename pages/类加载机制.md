...
- 双亲委派模型
  ...
  **自底向上判断类是否被加载过**
  ...
  双亲委派模型好处:保证java程序核心类库的稳定运行。防止核心类库被非法篡改。
  ...
  java9引入模块化后微调，主体结构未变
	- 破坏双亲委派模型
	  虽然是java官方的强力推荐，到java9为此，也有过三次大规模的破坏.
		- 第一次:对历史的兼容性问题.
		  jdk1.2才引入双亲委派模型，而类加载器概念和ClassLoader的loadClass方法在之前就已经存在了。所以考虑到兼容性问题，无法避免loadClass方法被子类修改.对于已经存在的用户自定义的类加载器,设计双亲委派模型时做了一些妥协，添加一个新的protected方法findClass()，并引导用户编写的类加载逻辑时尽可能去重写这个方法，而不是在loadClass()中编写代码.
		- 第二次
		- 第三次
- 资料
