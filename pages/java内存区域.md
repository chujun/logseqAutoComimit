...
- java内存区域划分
	...
	- java内存区域比较汇总表
	  ...
	  |---|---|---|---|---|---|
	  |程序计数器|线程私有|虚拟机规范唯一没有规定抛出内存异常|一般很小|确定性|
	  |虚拟机栈|线程私有|OutOfMemoryError,StackOverFlowError| -Xss128k指定栈空间大小|局部变量,方法出口|确定性|
	  |本地方法栈|线程私有|OutOfMemoryError,StackOverFlowError||局部变量,方法出口|确定性|
	  |堆|线程共有|OutOfMemoryError|-Xmx最大值,-Xms 最小值|对象实例|不确定，垃圾回收器回收|
	  |方法区(逻辑概念上)|线程共享|OutOfMemoryError|||不确定，垃圾回收器回收|
